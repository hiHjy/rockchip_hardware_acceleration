...
struct rk_mpp_dma_heap_allocation_data {
    __u64 len;
    __u32 fd;
    __u32 fd_flags;
    __u64 heap_flags;
...
/*
 * 有些目标 rootfs 不带 linux/dma-heap.h。
 * 这里只保留最小 ioctl 定义，方便老版本 Rockchip sysroot 也能编译。
 */
#define RKMPP_DMA_HEAP_IOC_MAGIC 'H'
#define RKMPP_DMA_HEAP_IOCTL_ALLOC \
    _IOWR(RKMPP_DMA_HEAP_IOC_MAGIC, 0x0, struct rk_mpp_dma_heap_allocation_data)

typedef struct RkMppMjpegFrameInfo {
    /* JPEG 头解析出来的可见图像宽高，或者解析失败时的兜底宽高。 */
    RK_U32 width;
    RK_U32 height;

    /* 解码输出 NV12 使用的 stride，通常按硬件要求做 16 字节对齐。 */
    RK_U32 hor_stride;
    RK_U32 ver_stride;

    /* 当前 MJPEG 压缩包的真实有效字节数。 */
    size_t packet_size;

    /*
     * 输出 dma-buf 至少需要的大小。
     * 这里按 stride * aligned_height * 2 计算，给 MJPEG 常见输出格式留足空间。
     */
    size_t buffer_size;
} RkMppMjpegFrameInfo;
 ...
 * - rk_mpp_mjpeg_dump_nv12_frame_from_dmafd()
 *
 * 第 2 层：输入 / 输出 buffer 准备
 * - rk_mpp_mjpeg_get_frame_info_from_dmafd()
 * 第 3 层：解码器生命周期
 * - rk_mpp_mjpeg_decoder_init()
 * - rk_mpp_mjpeg_decoder_deinit()
 *
 * 第 4 层：真正执行一帧解码
 * - rk_mpp_mjpeg_decoder_decode_dmafd()
 *
...
static int mjpeg_parse_size_from_memory(const RK_U8 *data,
                                        size_t len,
                                        RK_U32 *width,
                                        RK_U32 *height)
{
...
static int rk_mpp_mjpeg_ioctl_retry(int fd, unsigned long request, void *arg)
{
    int ret;
...
static int rk_mpp_mjpeg_create_dmabuf_fd_from_drm(size_t size)
{
    /*
     * 内部输出 buffer 的第一选择。
     *
     * DRM dumb buffer 可以导出成 dma-buf，通常适合 MPP/RGA/显示之间传递。
     * PRIME 导出 fd 之后马上 destroy dumb handle；只要 dma-buf fd 还开着，
     * 背后的内存就仍然有效。
     */
    ...
    if (rk_mpp_mjpeg_ioctl_retry(drm_fd, DRM_IOCTL_MODE_CREATE_DUMB, &create_req) < 0) {
    ...
    if (rk_mpp_mjpeg_ioctl_retry(drm_fd, DRM_IOCTL_PRIME_HANDLE_TO_FD, &prime_req) < 0) {
        struct drm_mode_destroy_dumb destroy_req;

        perror("DRM_IOCTL_PRIME_HANDLE_TO_FD");
        memset(&destroy_req, 0, sizeof(destroy_req));
        destroy_req.handle = create_req.handle;
        rk_mpp_mjpeg_ioctl_retry(drm_fd, DRM_IOCTL_MODE_DESTROY_DUMB, &destroy_req);
        close(drm_fd);
        return -1;
    }
...
static int rk_mpp_mjpeg_create_dmabuf_fd_from_heap(size_t size)
{
    /*
     * DRM 路径不可用时的兜底方案。
     * 这里尝试 Rockchip 和主线内核里常见的 dma_heap 名称。
     */
    ...
    struct rk_mpp_dma_heap_allocation_data alloc;
    int heap_fd = -1;
    int dma_fd;
    size_t i;
    ...
    if (ioctl(heap_fd, RKMPP_DMA_HEAP_IOCTL_ALLOC, &alloc) < 0) {
...
static int rk_mpp_mjpeg_create_dmabuf_fd(size_t size)
{
    /*
     * 内部输出 fd 的申请策略：
     * 1. 优先走 DRM dumb buffer 导出，方便后面接显示/RGA。
     * 2. DRM 失败时退回 dma_heap，方便无显示或设备节点不同的系统继续运行。
     */
    int fd;

    fd = rk_mpp_mjpeg_create_dmabuf_fd_from_drm(size);
    if (fd >= 0)
        return fd;

    return rk_mpp_mjpeg_create_dmabuf_fd_from_heap(size);
}

static int rk_mpp_mjpeg_prepare_internal_output(RkMppMjpegDecoder *ctx,
                                                int index,
                                                size_t required_size)
{
    /*
     * 只有调用方传 dstfd < 0 时才会走到这里。
     *
     * decoder 按 index 缓存内部输出 fd：
     * - 当前 fd 容量够，就直接复用，避免每帧申请；
     * - 当前 fd 太小，就关闭旧 fd，再申请一块更大的。
     */
    int new_fd;

    if (!ctx || required_size == 0)
        return -1;
    if (index < 0 || index >= RKMPP_MJPEG_OUTPUT_SLOT_COUNT) {
        fprintf(stderr, "[MPP] internal output needs index in [0, %d), got %d\n",
                RKMPP_MJPEG_OUTPUT_SLOT_COUNT, index);
        return -1;
    }

    if (ctx->dst_fd[index] >= 0 && ctx->dst_size[index] >= required_size)
    {
        printf("[MPP] reuse internal output fd=%d index=%d size=%zu required=%zu\n",
               ctx->dst_fd[index], index, ctx->dst_size[index], required_size);
        return 0;
    }

    new_fd = rk_mpp_mjpeg_create_dmabuf_fd(required_size);
    if (new_fd < 0) {
        fprintf(stderr, "[MPP] failed to create internal output dmabuf size=%zu\n",
                required_size);
        return -1;
    }
...
static int rk_mpp_mjpeg_get_frame_info_from_dmafd(int fd,
                                                  size_t packet_size,
                                                  RkMppMjpegFrameInfo *info)
{
    /*
     * 这一步很关键：
     *
    ...
    if (mjpeg_parse_size_from_memory((const RK_U8 *)addr, packet_size, &width, &height) != 0)
    {
...
static int rk_mpp_mjpeg_dump_nv12_frame_from_dmafd(const char *path,
                                                   int fd,
                                                   RK_U32 width,
                                                   RK_U32 height,
                                                   RK_U32 hor_stride,
                                                   RK_U32 ver_stride)
{
    ...
    {
        if (fwrite(uv_plane + (size_t)row * hor_stride, 1, width, fp) != width)
        {
...
void rk_mpp_mjpeg_decoder_deinit(RkMppMjpegDecoder *ctx)
{
    ...
    for (int i = 0; i < RKMPP_MJPEG_OUTPUT_SLOT_COUNT; ++i)
    {
        if (ctx->dst_fd[i] >= 0)
        {
            close(ctx->dst_fd[i]);
...
int rk_mpp_mjpeg_decoder_decode_dmafd(RkMppMjpegDecoder *ctx, int fd, int dstfd, int index, int w, int h, int stride, int size)
{
    /*
     * 这是整条 MPP 解码链路的核心函数。
     *
     ...
     *
     * 输出：
     * - dstfd >= 0 时，MPP 把 NV12 写入调用方提供的 fd。
     * - dstfd < 0 时，内部按 index 申请/复用输出 dma-buf。
     *
    ...
    if (rk_mpp_mjpeg_get_frame_info_from_dmafd(fd, (size_t)size, &info) != 0)
    {
        /*
        ...
        if (w <= 0 || h <= 0)
        {
            fprintf(stderr, "[MPP] failed to parse mjpeg header and no fallback size is available\n");
            return -1;
        }
        ...
        printf("[MPP] fallback to v4l2 size w=%u h=%u hs=%u vs=%u packet=%zu\n",
               info.width, info.height, info.hor_stride, info.ver_stride, info.packet_size);
    }
    else if (info.width != (RK_U32)w || info.height != (RK_U32)h)
    {

        printf("[MPP] jpeg header size %ux%u differs from v4l2 %dx%d, trust jpeg header\n",
               info.width, info.height, w, h);
    } else {
        printf("[MPP]从码流获取到头部信息成功:size w=%u h=%u hs=%u vs=%u packet=%zu\n",
               info.width, info.height, info.hor_stride, info.ver_stride, info.packet_size);
    }
    ...
    if (output_fd < 0)
    {
        /*
         * 调用方没有提供输出 fd。
         * 这里按 index 申请/复用一块内部 dma-buf，然后把 NV12 解到这个 fd。
         * 对内部 fd 来说，回调就是交给后续链路使用的出口。
         */
        if (rk_mpp_mjpeg_prepare_internal_output(ctx, index, output_size) != 0)
            return -1;
        output_fd = ctx->dst_fd[index];
        printf("[MPP] using internal output fd=%d index=%d size=%zu\n",
               output_fd, index, ctx->dst_size[index]);
    }
    else
    {
        /*
         * 调用方提供了目标 dma-buf。
         * 这个 fd 不存进 ctx，也不会在 deinit 里关闭；所有权完全留在调用方。
         * 调用方需要保证它至少能容纳 info.buffer_size 字节。
         */
        printf("[MPP] using external output fd=%d index=%d size=%zu\n",
               output_fd, index, output_size);
    }
    ...
    printf("[MPP] decoded frame index=%d w=%u h=%u hs=%u vs=%u fmt=%u buf=%zu err=%u discard=%u dst_fd=%d\n",
           index,
           ...
           mpp_frame_get_fmt(frame_out),
           mpp_frame_get_buf_size(frame_out),
           mpp_frame_get_errinfo(frame_out),
    ...
    if (mpp_frame_get_errinfo(frame_out) == 0 && mpp_frame_get_discard(frame_out) == 0)
    {
        if (ctx->frame_callback != NULL)
        {
            ctx->frame_callback(output_fd,
                                index,
                                mpp_frame_get_width(frame_out),
                                mpp_frame_get_height(frame_out),
                                mpp_frame_get_hor_stride(frame_out),
...
int rk_mpp_mjpeg_decoder_init(RkMppMjpegDecoder *ctx, MppCodingType coding)
{
    ...
    if (ret != MPP_OK)
    {
        fprintf(stderr, "[MppProxy] mpp_create(dec) failed: %d\n", ret);
        return -1;
    }
    ...
    if (ret != MPP_OK)
    {
        fprintf(stderr, "[MppProxy] mpp_init(dec) failed: %d\n", ret);
        mpp_destroy(ctx->dec_ctx);
        ctx->dec_ctx = NULL;
        ctx->dec_api = NULL;
        return -1;
    ...
    if (ret == MPP_OK && dec_cfg != NULL)
    {
        ...
        mpp_dec_cfg_set_u32(dec_cfg, "base:split_parse",
                            coding == MPP_VIDEO_CodingMJPEG ? 0 : 1);
        ctx->dec_api->control(ctx->dec_ctx, MPP_DEC_SET_CFG, dec_cfg);
        ctx->dec_api->control(ctx->dec_ctx, MPP_DEC_SET_OUTPUT_FORMAT, &output_fmt);
        mpp_dec_cfg_deinit(dec_cfg);
    }

    ctx->dec_initialized = 1;
    ctx->buf_is_init = 0;
    printf("[MppProxy] Decoder initialized (codec=%d)\n", coding);
    return 0;
}
